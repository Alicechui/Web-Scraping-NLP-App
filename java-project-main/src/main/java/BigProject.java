...
//        setting a label for URL
        ...
        wordLabel.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 15));
...
//                get the url and the search word
                ...
                if (url.isEmpty() ){
                    throw new NullPointerException("Please enter a file name or a url");
...
//                else add the sentence to the end of the @output
    ...
    private void showPosTag(CorpusBuilder cb, String PosTag) throws BadLocationException {
        for (List<String> sents : cb.getTokens())
            while (sents.equals(PosTag)) {
//                update the keyword to prevent the loop from being infinite
                sents.set(sents.indexOf(PosTag), PosTag.toUpperCase());
//                if there is nothing in the output - which is of Document value - put a nicely joined first sentence
                if (output.getLength() == 0)
                    output.insertString(0, String.join(" ", sents)+"\n", null);
//                else add the sentence to the end of the @output
    ...
    private void findLemma(CorpusBuilder cb, String PosTag) throws BadLocationException {
